...
import { LayoutDashboard, Users, Package, ShoppingCart, FolderTree, Ticket, Settings, FileBarChart, Store, LogOut, X, ChevronLeft } from 'lucide-react';
      ...
      <aside className={cn('fixed left-0 top-0 z-40 flex h-screen flex-col bg-[#15110d] transition-all duration-300 border-r border-white/[0.06]', collapsed ? 'w-20' : 'w-64', onClose ? 'lg:translate-x-0' : 'translate-x-0')}>
        <div className="flex h-16 shrink-0 items-center justify-between px-4">
          <div className="flex items-center gap-2.5 overflow-hidden">
            ...
            {!collapsed && <span className="font-editorial text-lg font-medium tracking-tight text-[#f0e9dd]">Shop<span className="italic text-primary-400">Verse</span></span>}
          </div>
        ...
        <div className="px-4 pb-2">
          {!collapsed && <p className="text-[0.65rem] font-semibold uppercase tracking-[0.16em] text-white/30">{user?.role === 'ADMIN' ? 'Administration' : 'Seller Console'}</p>}
        </div>
        <nav className="flex-1 space-y-0.5 overflow-y-auto scrollbar-thin px-3 py-2">
          {nav.map((i) => { const I = i.icon; return (
            <NavLink key={i.to} to={i.to} onClick={onClose} className={({ isActive }) => cn('group relative flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-all duration-200', isActive ? 'bg-white/[0.07] text-primary-300' : 'text-white/55 hover:bg-white/[0.04] hover:text-white/90', collapsed && 'justify-center')}>
              {({ isActive }) => (
                <>
                  {isActive && <span className="absolute left-0 top-1/2 h-5 w-[3px] -translate-y-1/2 rounded-r-full bg-primary-400" />}
                  <I className="h-[18px] w-[18px] shrink-0" strokeWidth={1.75} />{!collapsed && <span>{i.label}</span>}
                </>
              )}
            </NavLink>
          ); })}

...
import { SlidersHorizontal, Grid, List, X, Package, Search, Sparkles } from 'lucide-react';
...
export default function ProductsPage() {
  ...
  const loadCategoriesAndBrands = useCallback(async () => {
    try {
      ...
      // API returns a nested tree — flatten it so the sidebar list below
  ...
  const CategoryList = () => (
    <div>
      ...
      <div className="space-y-0.5">
        <button onClick={() => updateParam('categoryId', '')} className={cn('flex w-full items-center justify-between rounded-md px-3 py-2 text-left text-sm transition-colors', !categoryId ? 'bg-primary text-primary-foreground font-medium' : 'text-muted-foreground hover:bg-accent hover:text-foreground')}>All Products</button>
        {categories.map((c) => (
          <button key={c.id} onClick={() => updateParam('categoryId', String(c.id))} className={cn('flex w-full items-center justify-between rounded-md px-3 py-2 text-left text-sm transition-colors', categoryId === String(c.id) ? 'bg-primary text-primary-foreground font-medium' : 'text-muted-foreground hover:bg-accent hover:text-foreground')} style={{ paddingLeft: `${0.75 + (c.depth || 0) * 0.85}rem` }}>
            {c.depth ? `— ` : ''}{c.name}
          </button>
        ))}
      </div>
      ...
      <div className="mx-auto max-w-7xl px-4 pt-8 sm:px-6 lg:px-8">
        <div className="grid gap-8 lg:grid-cols-[240px_1fr]">
          ...
          <aside className="hidden lg:block">
            <div className="sticky top-36 space-y-7 rounded-lg border border-border bg-card p-5">
              <CategoryList />
              <div className="divider-fade" />
              <RefinePanel />
            </div>
